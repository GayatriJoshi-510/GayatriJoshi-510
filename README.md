![header](https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=200&section=header&text=Gayatri%20Joshi&fontSize=50&fontColor=fff&animation=twinkling&fontAlignY=35&desc=BCA%20Student%20|%20Developer%20|%20ML%20Enthusiast&descAlignY=55&descSize=18)

# Hi, I'm Gayatri Joshi 👋

🎓 BCA (Bachelor of Computer Applications) Student  
💻 Passionate about Python, Java, Web Development & Cybersecurity  
🤖 Built an Intelligent Health Assessment System using Machine Learning  
🔍 Interested in AI, NLP, and Full Stack Development  

🛠️ Skills
- Python | Java | HTML | CSS | JavaScript
- Machine Learning | NLP | Scikit-Learn | Flask | Streamlit
- SQLite | MySQL | Tkinter | SHA-256 Cryptography

📂 My Projects
- 🏥 Intelligent Health Assessment System - AI-powered disease prediction web app
- 🔍 Fake Job Detector - NLP-based fraudulent job posting classifier
- 🔐 Cybersecurity Tools - Password manager with SHA-256 hashing
- ☕ Java Student Management System - Desktop GUI application
- 🌐 Web Development Projects - Digital Clock, To-Do List & more

📫 Connect with Me
- GitHub: [GayatriJoshi-510](https://github.com/GayatriJoshi-510)

🌸 "Every project I build is more than code—it's proof that I never stop learning, improving, and believing in what's possible."
![footer](https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=100&section=footer)
